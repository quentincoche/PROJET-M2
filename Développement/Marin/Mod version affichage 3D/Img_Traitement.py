...
class Traitement():
    ...
    def trace_profil(self,dpi,width,height, pixel_size):
        ...
        fig.suptitle("Gaussiennes x,y $\tau$")
    ...
    def plot_2D(self,dpi,width,height):
        ...
        """
        a=((math.cos(math.pi/180*ang_ell-0.5*math.pi)**2)/(2*sigma_g**2)+(math.sin(math.pi/180*ang_ell-0.5*math.pi)**2)/(2*sigma_p**2))
        b=(math.sin(2*math.pi/180*ang_ell-0.5*math.pi)/(2*sigma_g**2)-math.sin(2*math.pi/180*ang_ell-0.5*math.pi)/(2*sigma_p**2))
        c=((math.sin(math.pi/180*ang_ell-0.5*math.pi)**2)/(2*sigma_g**2)+(math.cos(math.pi/180*ang_ell-0.5*math.pi)**2)/(2*sigma_p**2))
        z=amp*np.exp(-(a*(xi-x0)**2+2*b*(xi-x0)*(yi-y0)+c*(yi-y0)**2))
        """
        #paramètres pour affichage correct
        fig_width_i = width / dpi
        fig_height_i = height / dpi
        ...
        cs = ax3.plot_surface(xi, yi, eps + model_data, cmap="coolwarm",
                       linewidth=0, antialiased=True, label="Modèle Gaussien 2D")
