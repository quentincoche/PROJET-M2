...
while (exp_time_ok==0) % boucle tant que expo time non ok
    ...
    img = double((obj));
     ...
     if (max(max(img)) >= 254)   % Si saturation, reduction tps exp
        ...
        strg=strcat('Exp time uncal+ \',num2str(src.ExposureTime));
     ...
     elseif (max(max(img)) <= 160) % Si low level, augmentation tps exp
        ...
        exp_time_ok=1;
        strg=strcat('Exp time OK \',num2str(src.ExposureTime));
    ...
    if  src.ExposureTime >= 2000000  % test sur tps d'expo trop long
            ...
            exp_time_ok=1;
            strg=strcat('EXP TIME TOO BIG !\',num2str(src.ExposureTime));
            set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string',strg,'enable','on')
     end
     ...
     if  src.ExposureTime <= 20
            src.ExposureTime=20;
            exp_time_ok=1;
            strg=strcat('EXP TIME TOO LOW !\',num2str(src.ExposureTime));
            set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string',strg,'enable','on')
     end
end
