...
%-----------------------------
% Auteurs: Josselin Martin, Julien Fade, Goulc'hen Loas
% Version: 15 nov. 2012
% Bug corrections: 
...
function varargout = IPR_DO_beam_analyzer(varargin);
warning off
disp({ '======================================================================';
       '=                      IPR_DO_beam_analyzer                          =';
       '======================================================================';
...
global hold_res img_res
global X0 Y0 X0r Y0r
...
%Création de l'objet Align
uicontrol(fig1,'style','pushbutton',...
    'units','normalized',...
    'position',[0.9 0.40 0.1 0.04],...
    'string','ALIGN',...
    'callback',@align);

%Création de l'objet Hold
uicontrol(fig1,'style','pushbutton',...
    'units','normalized',...
    'position',[0.9 0.35 0.1 0.04],...
    'string','HOLD',...
    'callback',@hold_beam_res);
...
function hold_beam_res(hObject,data)
global X0r Y0r X0 Y0 
% Sauve la fenetre resultat principale pour recall ultérieur
...
ma_vid=videoinput('gentl', 1, 'Mono8');
...
while (exp_time_ok==0) % boucle tant que expo time non ok
    ...
    img = double((obj));
     ...
     if (max(max(img)) >= 254)   % Si saturation, reduction tps exp
        src.ExposureTime = floor(src.ExposureTime /2);
        set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string','Exp time uncal+','enable','on');
    
     elseif (max(max(img)) <= 160) % Si low level, augmentation tps exp
        
        src.ExposureTime = ceil(src.ExposureTime * 1.5);
        set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string','Exp time uncal-','enable','on');
     
    else
        exp_time_ok=1;
        set(text,'BackgroundColor',[0 0 0],'ForegroundColor',[1 0 0],'enable','off'); %Message d'erreur masqué quand il n'y a aucun problème
     end
     
    if  src.ExposureTime >= 2000000  % test sur tps d'expo trop long
            src.ExposureTime=2000000;
            exp_time_ok=1;
            set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string','EXP TIME TOO BIG !','enable','on')
     end
     
     if  src.ExposureTime <= 20
            src.ExposureTime=20;
            exp_time_ok=1;
            set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string','EXP TIME TOO LOW !','enable','on')
     end
end
...
function [X0,Y0]=bary(hObject,data); 
...
set(text,'BackgroundColor',[1 0 0],'ForegroundColor',[1 1 1],'string','PB ESTI BCKG!','enable','on');   
end
...
l1=line([X0-50 X0+50],[Y0 Y0]);
l2=line([X0 X0],[Y0-50 Y0+50]);
l1r=line([X0r-50 X0r+50],[Y0r Y0r]);
l2r=line([X0r X0r],[Y0r-50 Y0r+50]);
...
while (cnt<=1);
 ...
 %% Définition de g et test de g pour savoir si le calcul des coefficients de l'ellipse est possible
    ...
    if  (g<=0)
        ...
        set(text,'string','CALC ELL IMP !','enable','off');
        ...
        img_res=zeros(size(img,1),size(img,2),3);
        img_res(:,:,1)=(double(img)/255).*(1-c_contour_ell) + c_contour_ell;
        img_res(:,:,2)=(double(img)/255).*(1-c_contour.*(1-c_contour_ell)) + c_contour.*(1-c_contour_ell);
        img_res(:,:,3)=(double(img)/255).*(1-c_contour_ell.*c_contour);
...
%          end
       %% Tracé du grand axe
        ...
        % impossible)
        set(text,'string','CALC ELL IMP !','enable','on');
        cnt=2;
    end
...
c_contour_ell = (imgnorm < (pres(2)/exp(1)^2+ delta) ) .* ( imgnorm >(pres(2)/exp(1)^2-delta ) ); 
        ...
        img_res=zeros(size(imfit,1),size(imfit,2),3);
        img_res(:,:,1)=(double(img)/255).*(1-c_contour_ell) + c_contour_ell;
        img_res(:,:,2)=(double(img)/255);
        img_res(:,:,3)=(double(img)/255);
...
%% Affichage image + fit sur fenêtre principale
    ...
    size(img_res)
    size(hold_res)
    imagesc((img_res+hold_res)/2);daspect([1 1 1]);
    line(X,Y);
    line(Xp,Yp);
